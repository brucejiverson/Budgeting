...
def month_expenses(month, year, df):
    ...
    ax1.bar([0, 1], [income, -expenses], width=width, color=['green', 'red'])
    plt.sca(ax1)
    ...
    ax1.text(0, income/2, '$%.2f' % my_dict['Income'], horizontalalignment='center', verticalalignment='center')
    ax1.text(1, -expenses/2,  '$%.2f' %
             my_dict['Expenses'], horizontalalignment='center', verticalalignment="center")
    # label with the values and the difference

    main_expenses = pd.DataFrame(data={'Expenses': [-shopping, -groceries, -restaurants, -life_costs, -other]}, index=[
...
transactions.rename(index=str, columns={" Amount ": "Amount"}, inplace=True)
...
month_expenses(11, 2018, transactions)
# month_expenses(11, 2018, transactions)
